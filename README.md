...
- `src/store` holds client-only state: `clock` (ticks every second while the app is active, pauses in background), and `preferences` (theme `auto|light|dark` resolved by `resolveTheme`, `showSecondHand`). Components use `useAppSelector` / `useAppDispatch` from `@/store`; the root layout runs `useClock` and `useThemeSync`. Sheets are routes and the correction day rides on `?day=`, so there is no UI slice.
