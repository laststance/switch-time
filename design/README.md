...
- `switch-time.pen` — 全フレーム（ST Phone 7 枚＋訂正シートの持ち越し行 2 枚・ST Menubar・ST Web 2 枚＋訂正シートの状態行・設定のタイムゾーン行の状態・ホームの detox の状態の各ボード＋サインイン（登録後）1 枚）、変数 37 個、`mode: light / dark` のテーマ軸
