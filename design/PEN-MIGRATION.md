...
現在: 電話7枚と訂正シートの持ち越し行2枚が `y=0`（x を 462 刻み）、サインイン（登録後）（ST Phone / サインイン（登録後）、x=4966）も `y=0`、Menubar と Web 2枚（ST Web・ST Web / 訂正シート）と訂正シートの状態行ボード（ST Phone / 訂正シート・状態行、x=4966）が `y=1558`、その下の x=4966 に設定のタイムゾーン行の状態（`y=2115`）とホームの detox の状態（ST Phone / ホーム・detox の状態、`y=2399`）。
