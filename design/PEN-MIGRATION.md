...
**決定（2026-09-07）: 近似として受け入れる。**
正は `design-system/styles.css` のまま、pen 側は描画上の近似という扱い。
Roboto はスタックの Android 項そのものなので嘘ではない。**再燃させないこと。**
...
現在: 電話7枚と訂正シートの持ち越し行2枚が `y=0`（x を 462 刻み）、サインイン（登録後）（ST Phone / サインイン（登録後）、x=4966）も `y=0`、Menubar と Web 2枚（ST Web・ST Web / 訂正シート）と訂正シートの状態行ボード（ST Phone / 訂正シート・状態行、x=4966）が `y=1558`、その下の x=4966 に設定のタイムゾーン行の状態（`y=2115`）とホームの detox の状態（ST Phone / ホーム・detox の状態、`y=2399`）。訂正シートのタップのない日の注記（ST Phone / 訂正シート・タップのない日の注記）は持ち越し行の状態と同じ x=3696 の `y=2345`、その下に自分の行の区切る時刻の状態（ST Phone / 訂正シート・区切る時刻の状態、`y=3295`）と Control の押下とフォーカス（ST Phone / Control の押下とフォーカス、`y=4166`）。
