...
| `face` | `#FFFFFF` | `#212229` | 時計盤面 ※**乗っている面から1段離す**（Phone/menubar は `bg`、Web は `surface`）。影が無いので値差が輪郭 |
| `surface` | `#FFFFFF` | `#1B1C22` | カード・非選択ボタン |
| `sheetBg` | `#FFFFFF` | `#1C1D22` | モーダル/シート |
| `ink` | `#1B1A17` | `#F2EFE8` | 本文 |
| `sub` | `rgba(27,26,23,0.64)` | `rgba(242,239,232,0.6)` | 補助テキスト |
...
| `hatch` | `repeating-linear-gradient` の斜線（枠は `sub`） | **`1.5px dashed {sub}` ＋ `chip` の塗り**（`sub` なので `surface` に対して 3:1 を超える） |
...
経過時間の数字は tabular-nums。スケール: **10 / 12 / 15 / 18 / 22 / 28 / 40px**、＋ヒーローの経過時間だけ `--text-display: 52px`。
段差は上に行くほど広がる（2/3/3/4/6/12）。旧 14/16/18/20 は +2px の等差で、4サイズが1つに見えていた。
...
1. **`face` は「乗っている面」から1段浮かせる**（上表）。影を外した以上、値差が盤面の輪郭になる。
   Phone / menubar の地は `bg`。**Web だけは `surface` のカードの上**に乗り、light では両方 `#FFFFFF` で同値になる —
   ここはリムとベゼルリングが輪郭を持つ（実寸で確認済み）。dark はリングだけでは持たないので値差を保つこと。
2. **縁はリム、影ではない。** 盤面の外周は `line` のヘアライン1本。
   **`filter: drop-shadow(...)` は使わない** — 浮いたシールに見えるうえ、react-native-web が `filter` を落とすので Web では消える。
3. **活動リングはベゼル線。** 盤面外周 1.5px、現在の活動色。4px だと画面で一番うるさい物体になり、計器に見えなくなる。
   活動色は中心ピン・見出し・押されたボタンの塗りが既に持っているので、リングは縁を示すだけでよい。
4. **目盛りは3段階。** 12/3/6/9 が最も長く太い → 残りの時 → 分はヘアライン（`line`）。
...
ステータスの点（中抜き）、経過時間は `sub` に落ちる。24h バーと訂正シートのバーの detox スパン、訂正シートの detox 行のチップ（アイコンは `ink`）、記録の「計測できて活動の時間が無く、detox が放置（idle）以上だった日」（タップの無い日でも、detox が通り抜けた日を、その detox を始めた日の7日後まで含む）のセルは
塗らずに `sub` の**実線**の輪郭で描き、記録のセルには中央に `sub` の wind アイコン（週 14px・月 12px）を置く。
それ以外の日（計測なしの日も）の detox の時間は、活動の塗りの上に積む `sub` の実線 1px の区間として描く（2px 未満は描かない、24h の枠からはみ出す分は切る）。活動の塗りも同じで、25 時間の日（夏時間の終わり）は 24h の枠を超える分を切る。切った時間もセルの読み上げには入る。角丸は 0.5px 以上の区間の上下端に付け、見えない細い区間には渡さない。
状態別では活動の後に detox の行（輪郭のチップ、合計、1日あたり、目安が無いのでバーは空）を置く。計測できた日に detox が無ければ出さない。
破線は「データが無い」印に限る: 記録では `hatch`（未計測日、枠は `sub`）、24h バーでは放置（idle）のスパン。detox は形（実線）と色（`sub`）の両方で区別するので、計測なしの日とも放置とも混ざらない。

選択中のカード（`chip` 塗り）の中に出す注意の枠は **`sheetBg` 塗り＋`line` の枠＋角丸 10、文字は `ink` 12px**。`chip` の箱は同じ `chip` の上で消えるので使わない（訂正シートの持ち越し行: アーカイブ済みの活動の警告と通知）。
