...
それ以外の日（計測なしの日も）の detox の時間は、活動の塗りの上に積む `sub` の実線 1px の区間として描く（2px 未満は描かない、24h の枠からはみ出す分は切る）。活動の塗りも同じで、25 時間の日（夏時間の終わり）は 24h の枠を超える分を切り、0.5px 未満の区間は描かない。切った時間もセルの読み上げには入る。
