...
ステータスの点（中抜き）、経過時間は `sub` に落ちる。24h バーと訂正シートのバーの detox スパン、訂正シートの detox 行のチップ（アイコンは `ink`）、記録の「計測できて全部 detox だった日」（タップの無い日でも detox が通り抜けた日を含む）のセルは
塗らずに `sub` の**実線**の輪郭で描き、記録のセルには中央に `sub` の wind アイコン（週 14px・月 12px）を置く。
破線は「データが無い」印に限る: 記録では `hatch`（未計測日）、24h バーでは放置（idle）のスパン。detox は形（実線）と色（`sub`）の両方で区別するので、計測なしの日とも放置とも混ざらない。
