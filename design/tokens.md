...
ステータスの点（中抜き）、経過時間は `sub` に落ちる。24h バーと訂正シートのバーの detox スパン、訂正シートの detox 行のチップ（アイコンは `ink`）、記録の「計測できて活動の時間が無く、detox が放置（idle）以上だった日」（タップの無い日でも detox が通り抜けた日を含む）のセルは
