...
ステータスの点（中抜き）、経過時間は `sub` に落ちる。24h バーと訂正シートのバーの detox スパン、訂正シートの detox 行のチップ（アイコンは `ink`）、記録の「計測できて活動の時間が無く、detox が放置（idle）以上だった日」（タップの無い日でも、detox が通り抜けた日を、その detox を始めた日の7日後まで含む。自動除外がオンのとき、7日を過ぎて detox 行を押し直すと、その日から新しく数える）のセルは
塗らずに `sub` の**実線**の輪郭で描き、記録のセルには中央に `sub` の wind アイコン（週 14px・月 12px）を置く。
それ以外の日（計測なしの日も）の detox の時間は、活動の塗りの上に積む `sub` の実線 1px の区間として描く（2px 未満は描かない、24h の枠からはみ出す分は切る）。
