...
ステータスの点（中抜き）、経過時間は `sub` に落ちる。24h バーと訂正シートのバーの detox スパン、訂正シートの detox 行のチップ（アイコンは `ink`）、記録の「計測できて活動の時間が無く、detox が放置（idle）以上だった日」（タップの無い日でも、detox が通り抜けた日を、その detox を始めた日の7日後まで含む。7日を過ぎて detox 行を押し直すと、その日から新しく数える）のセルは
